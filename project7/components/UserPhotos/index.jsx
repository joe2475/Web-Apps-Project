...
function UserPhotos({userId}) {
  ...
  return(
    <div className="photoGroup">
      ...
      <>
      {typeof(photos) == undefined ?  <div className="photoGroup">
        <Typography variant="h3">User has no photos.</Typography>
      </div> : photos.map((photo) => {return( <div key={photo._id}>{<PhotoUnit photo={photo} user={user} setPhotos={setPhotos}/>}<br/><Divider/><br/></div>);})}</>}
    </div>
  );
}
